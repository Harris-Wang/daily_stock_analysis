...
# ── 韭研公社数据拉取（供 pipeline + analyzer 共用）────────────────────
def _fetch_jiuyan_context_for_pipeline(code: str, name: str = "") -> str:
    """从 signals.db 拉取该股最新韭研公社异动分析，供 analyze_stock 注入 prompt。"""
    try:
        import sqlite3
        conn = sqlite3.connect("E:/quant/minimax_quant/data/signals.db")
        cur = conn.cursor()
        parts = []
        # 异动解析（最重要）
        cur.execute(
            "SELECT trade_date, reason, analysis, industry "
            "FROM jiuyan_action WHERE code=? ORDER BY trade_date DESC LIMIT 1",
            (code,)
        )
        row = cur.fetchone()
        if row:
            parts.append(
                f"【韭研公社·异动解析】({row[0]}) 板块：{row[3]}\n"
                f"异动原因：{row[1]}\n详细分析：{row[2]}"
            )
        # 时间轴事件
        cur.execute(
            "SELECT event_time, title, content, concepts "
            "FROM jiuyan_timeline WHERE related_stocks_json LIKE ? "
            "ORDER BY event_time DESC LIMIT 3",
            (f"%{code}%",)
        )
        for r in cur.fetchall():
            parts.append(
                f"【韭研公社·时间轴事件】{r[0]} {r[1]}\n"
                f"概念标签：{r[3]}\n内容：{str(r[2])[:200]}"
            )
        conn.close()
        if parts:
...
class StockAnalysisPipeline:
    ...
    def analyze_stock(self, code: str, report_type: ReportType, query_id: str) -> Optional[AnalysisResult]:
        ...
        try:
            ...
            # ── 注入韭研公社异动数据 ──────────────────────────────────
            # 从 signals.db 拉取该股最新韭研公社异动分析，
            # 挂到 enhanced_context.jiuyan_context，analyzer._format_prompt 会读到
            jiuyan_block = _fetch_jiuyan_context_for_pipeline(code, stock_name)
            if jiuyan_block:
                enhanced_context["jiuyan_context"] = jiuyan_block
                logger.info(f"{stock_name}({code}) 注入韭研数据 {len(jiuyan_block)} 字符")
    ...
    def _enhance_context(
        ...
        # ── 韭研公社数据注入 ────────────────────────────────────
        # 在 LLM 分析前，将韭研公社的逻辑归因注入上下文，供 AI 参考
        try:
            import sys as _sys
            _sys.path.insert(0, "E:/quant/minimax_quant")
            from sequoia_x.data.jiuyan_query import JiuyanQuery
            _jqr = JiuyanQuery()
            _code = context.get("code", "")
            if _code:
                # 从 signals.db 读该股在韭研的所有记录
                _jy_action = _jqr.query_action(code=_code, limit=10)
                _jy_comm   = _jqr.query_community(code=_code, limit=5)
                _jy_tl      = _jqr.query_timeline(code=_code, limit=5)
                if _jy_action or _jy_comm or _jy_tl:
                    _jy_lines = ["### 韭研公社数据（逻辑归因参考）"]
                    if _jy_action:
                        _ind_list = [a.get("industry","") for a in _jy_action[:3] if a.get("industry")]
                        _reason_list = [a.get("reason","") for a in _jy_action[:3] if a.get("reason")]
                        _jy_lines.append(f"- 异动题材：{' / '.join(filter(None, _ind_list)) or '暂无'}")
                        _jy_lines.append(f"- 异动原因：{' / '.join(filter(None, _reason_list)) or '暂无'}")
                    if _jy_comm:
                        _titles = [c.get("title","") for c in _jy_comm[:2] if c.get("title")]
                        _jy_lines.append(f"- 社群热议：{' | '.join(_titles) if _titles else '暂无'}")
                    if _jy_tl:
                        _concepts = [t.get("concepts","") for t in _jy_tl[:3] if t.get("concepts")]
                        _jy_lines.append(f"- 题材概念：{' / '.join(filter(None, _concepts)) or '暂无'}")
                    if len(_jy_lines) > 1:
                        enhanced["jiuyan_context"] = "\n".join(_jy_lines)
                        logger.debug(f"{_code} 注入韭研上下文: {len(_jy_action)}条异动")
        except Exception as _e:
            logger.debug(f"韭研公社数据注入失败（不影响主流程）: {_e}")
    ...
    def _agent_result_to_analysis_result(
        ...
        """
        report_language = normalize_report_language(getattr(self.config, "report_language", "zh"))
        # 初始化时即用权威名称，后续 AI 可能覆盖但须通过 Tencent 逆向验证（见下方逻辑）
        ...
        # 名称权威链：
        #   ① 传入的 stock_name（已通过 fetcher_manager / realtime_quote 验证，最权威）
        #   ② Tencent 实时行情 API（运行时核验，防止 fetcher 缓存失效）
        #   ③ AI 返回的 ai_stock_name（仅作最终 fallback，且须通过 Tencent 逆向验证）
        # 关键：AI 可能 hallucinate 真实感中文名（"联合药业""纳芯微"），
        #       绝不能在 stock_name 已有效时直接信任 ai_stock_name。
        authoritative_name = stock_name
        tencent_name = self._fetch_name_from_tencent(code)
        ai_name_for_fallback = None
        if agent_result.success and agent_result.dashboard:
            dash = agent_result.dashboard
            ai_stock_name = str(dash.get("stock_name", "")).strip()
            # AI name 只在 authoritative name 本身是 placeholder 时才考虑
            if ai_stock_name and self._is_placeholder_stock_name(authoritative_name, code):
                # 再给 Tencent 一次机会
                if tencent_name and not self._is_placeholder_stock_name(tencent_name, code):
                    result.name = tencent_name
                elif ai_stock_name:
                    # 最后 fallback：AI name 必须通过 Tencent 逆向验证
                    if tencent_name and ai_stock_name == tencent_name:
                        result.name = ai_stock_name
                    # 否则拒绝 AI name，保持 placeholder，等待人工核实
    ...
    @staticmethod
    def _fetch_name_from_tencent(code: str) -> Optional[str]:
        """从腾讯实时行情 API 获取中文股名，用于运行时名称核验"""
        import urllib.request
        suffix = f"sh{code}" if code.startswith(("6", "5")) else f"sz{code}"
        url = f"https://qt.gtimg.cn/q={suffix}"
        try:
